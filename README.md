...
3. Elegí el repo `bookmap3d` que acabás de subir. Render va a detectar
   el `render.yaml` solo y te va a mostrar el servicio `bookmap3d` listo
   para crear.
...
5. Esperá el build (1-2 minutos). Cuando termine, Render te da una URL
   tipo `https://bookmap3d.onrender.com` — entrá ahí y listo, ya está
   corriendo para vos (o para cualquiera a quien le pases el link) desde
   el celu o donde sea.
